...
async fn test_update_user_details_success() {
    ...
    db.expect_update_user_details()
        ...
        .withf(move |uid, details| {
            *uid == user_id
                ...
                && details.phone_country_code.as_deref() == Some("+1")
                && details.phone_number.as_deref() == Some("5550100")
    ...
    let request = Request::builder()
        ...
        .header(CONTENT_TYPE, "application/x-www-form-urlencoded")
        .body(Body::from(
            "name=Updated+User&company=Example&github_url=https%3A%2F%2Fgithub.com%2Fupdated-user&phone_country_code=%2B1&phone_number=5550100&substack_url=https%3A%2F%2Fupdated.substack.com&youtube_url=https%3A%2F%2Fyoutube.com%2F%40updated&optional_notifications_enabled=true",
        ))
        .unwrap();
    let response = router.oneshot(request).await.unwrap();
    let (parts, body) = response.into_parts();
